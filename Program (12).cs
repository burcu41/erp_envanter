...
// 5. PUT: 13. GÜN - Ürün Bilgilerini Güncelleme (Server-Side Validasyon & Parameterized Query)
app.MapPut("/api/products", (UrunGuncelleDto dto) =>
{
    // Backend Doğrulama Kontrolleri (Zırh)
    if (dto.UrunID <= 0)
        return Results.BadRequest(new { success = false, message = "Geçersiz ürün kimliği." });

    if (string.IsNullOrWhiteSpace(dto.UrunAdi) || dto.UrunAdi.Trim().Length < 2)
        return Results.BadRequest(new { success = false, message = "Ürün adı boş bırakılamaz ve en az 2 karakter olmalıdır." });

    if (dto.BirimFiyat <= 0)
        return Results.BadRequest(new { success = false, message = "Birim fiyat 0 veya negatif olamaz." });

    if (dto.StokAdedi < 0)
        return Results.BadRequest(new { success = false, message = "Stok adedi negatif olamaz." });

    if (dto.KritikStokSeviyesi < 1)
        return Results.BadRequest(new { success = false, message = "Kritik stok eşiği en az 1 olmalıdır." });
    ...
    try
    {
        ...
        // SQL Injection Açığını Önleyen Parametreli Güncelleme Sorgusu
        ...
        var rows = command.ExecuteNonQuery();
        return rows > 0 ? Results.Ok(new { success = true, message = "Ürün güncellendi." }) : Results.NotFound();
    }
    catch (Exception ex)
    {
        return Results.BadRequest(new { success = false, message = ex.Message });
